...
`experiment_id`/`strategy_id`/`candidates`/`description`, each candidate's
...
not convention or recollection, is authoritative. {range_authority_note} Write one
`bbb_autoresearch_execution_plan.v1` conforming to `{plan_schema_path}` at `{result_path}`. For v3
sessions, use the version required by that schema and obey this immutable typed stage context
exactly; do not invent stages, dimensions, paths, geometries, or prerequisite evidence:
