...
- `analysis_artifact`: `claim_id` and non-empty `analysis_path` are required; `candidate_id`,
  ...
  `analysis_path` must equal that declared value exactly, or be `null` if you declared none; never disguise file evidence as `canonical_metric`, and never cite a supervisor-owned canonical file
