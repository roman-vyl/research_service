...
- [x] 1.1 Inspect the current EMA Pullback strategy/config shapes, live catalog shapes, and
  config-validation contracts; implement the fixture-reference and validation boundary without
  inventing strategy syntax. Absence of an operator-approved starting document SHALL NOT block
  APPLY; it blocks initialization of the controlled harness session that requires that document.
- [x] 1.2 Add exact schemas for `bbb_autoresearch_stage_contract.v1`,
...
- [x] 1.3 Add schema-equivalent manual/model validation for all v3 contracts, including exact keys,
...
- [x] 1.4 Add negative contract tests for missing/extra fields, unknown stages/dimensions,
...
- [x] 2.1 Add support for a complete operator-approved canonical starting-strategy fixture with
  ticker/timeframe, trigger, direction, exits, risk, setup/blocker/context structure, component
  IDs, instance IDs, and all required parameters intact. The concrete EMA100/EMA200/EMA500 fixture
...
- [x] 3.1 Implement a narrow strategy normalizer/comparator that locates bound components by stable
  identity, compares candidates with the applicable immutable reference, and never mutates or
  interprets the strategy.
- [x] 3.2 Enforce identity immutability for strategy ID, enabled/control envelope, ticker,
...
- [x] 3.3 Enforce `A_BASELINE` as exactly one candidate for one configured geometry, with both bound
  exit instances equal to the configured distance and every other field identical to the naked
  starting strategy.
- [x] 3.4 Enforce B batches as one referenced completed Phase-A geometry family, with every
  candidate preserving that exact geometry and existing batch comparability/window invariants.
- [x] 3.5 Enforce width-only B1, naked-reset/lookback-only B2, and width-plus-lookback-only B3;
  explicitly reject B1 width leakage into B2 and any unrelated candidate change before execution.
- [x] 3.6 Add focused positive/negative tests for reordered component arrays, valid typed changes,
...
- [x] 4.1 Bind v3 plans to active stage, starting/reference hashes, one geometry ID, exact allowed
  dimensions, and prerequisite disposition references before plan/request freeze.
- [x] 4.2 Persist accepted Phase-A references with geometry and canonical experiment/candidate/run,
...
- [x] 5.1 Insert stage-contract validation after plan/request shape validation and before plan
  freeze/execution intent, leaving the canonical executor and `src/research_service/**` untouched.
- [x] 5.2 Bind stage/reference context transitively through existing plan/request hashes and receipt
  validation without creating a second evaluator or changing receipt scientific meaning.
- [x] 5.3 Revalidate frozen v3 stage/reference bindings during interpretation, commit, and recovery;
  never reread the repository fixture for an active session.
- [x] 5.4 Add crash/retry tests proving no duplicate batch after valid receipt, immutable geometry
...
- [x] 6.1 Add the v3 controlled-session template contract for referencing an operator-approved
  canonical fixture, configured symmetric measurement geometries, and A/B bindings to existing
  descriptive/structural quality roles. Do not fabricate a concrete starting strategy when the
  operator fixture is not yet supplied.
- [x] 6.2 Give planning and interpretation workers compact explicit active-stage context, frozen
...
- [x] 6.3 Update `program.md`, the Strategy Specification Reference link chain, and only the required
...
- [x] 6.4 Update status/README output for v3 stage, configured/completed geometries, dispositions,
  and next available stages without adding research ranking or recommendation logic.
- [x] 6.5 Add prompt/template/status regression tests proving workers receive exact controls and
  cannot interpret Phase A as TP/SL optimization or B3 as mandatory.

## 7. Acceptance and verification

- [x] 7.1 Run all targeted AutoResearch contract, init, mutation, lifecycle, brokered-flow,
  recovery, provenance, quality-policy, prompt, and legacy-compatibility tests.
- [x] 7.2 Run `make verify`, `openspec validate --all --strict`, and `git diff --check`; confirm
  `src/research_service/**`, Strategy Engine/MDS/accounting/execution semantics, and unrelated
  OpenSpec changes remain untouched.
- [x] 7.3 Review the implementation against every acceptance scenario and confirm no generic
